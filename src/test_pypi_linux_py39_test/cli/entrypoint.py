...
from test_pypi_linux_py39_test import __version__
from test_pypi_linux_py39_test.cli import APP_NAME, __app_name__
from test_pypi_linux_py39_test.cli.logs import LogLevel, configure_logging
from test_pypi_linux_py39_test.loc import _
...
__all__: Final = ["test_pypi_linux_py39_test"]
...
def print_copyright(ctx: Context, _param: Option, value: bool) -> None:
    ...
    console = importlib.import_module("test_pypi_linux_py39_test.console")
...
def print_license(ctx: Context, _param: Option, value: bool) -> None:
    ...
    package_metadata = importlib.import_module(
        "test_pypi_linux_py39_test.package_metadata"
    )
    console.STDOUT.print(
...
def print_debug_info(ctx: Context, _param: Option, value: bool) -> None:
    ...
    console.STDOUT.print(
        importlib.import_module(
            "test_pypi_linux_py39_test.debug_info",
            __package__,
        ).gather_debug_info()
    )
    ctx.exit()
...
def test_pypi_linux_py39_test(**kwargs: Unpack[CLIArgsType]) -> None:
    """The Whiteprint CLI."""
    configure_logging(
